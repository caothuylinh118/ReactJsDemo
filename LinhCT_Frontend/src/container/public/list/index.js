...
class List extends Component {
    ...
    componentDidMount() {
        fetch(Common.api + Common.list_hotel + '?city=' + this.state.selectedCity + '&from_date=' + this.state.selectedFrom + '&to_date=' + this.state.selectedTo)
        .then(res => res.json())
        .then((data) => {
        console.log(data);
        this.setState({
        ...
        fetch(Common.api + Common.recommended_hotel + '?city=' + this.state.selectedCity + '&limit=3')
        .then(res => res.json())
        .then((data) => {
          console.log(data);
          this.setState({
            recommended_hotel: data.data,
            dateInFormated: this.props.match.params.from,
            dateOutFormated: this.props.match.params.to
          })
        })
        .catch(console.log)
